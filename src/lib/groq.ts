...
2. モーラ数を${moraCount}モーラに完全一致させる（リズム最優先・字余り厳禁）
3. 元フレーズ「${text}」に含まれる単語・文字を候補に一切使わない
...
■ 文体について:
正しい日本語文法に縛られなくていい。
口語・砕けた表現・ネットスラング・体言止め・倒置・省略——
モーラ数とリズムさえ合えば何でも使ってよい。
むしろ「正しい文章」にしようとして字余りになることの方が致命的失敗だ。
